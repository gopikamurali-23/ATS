...
const ApplicantLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // In backend, login accepts 'username' parameter which we have extended to check both email and username.
      const data = await login(email, password);
      if (data.role === 'APPLICANT') {
        navigate('/applicant/dashboard');
      } else {
        setError('Unauthorized role. This portal is for Job Seekers/Applicants only.');
        localStorage.clear();
      }
    } catch (err) {
      ...
      <div className="max-w-md w-full space-y-8 glass p-8 rounded-3xl shadow-xl border border-slate-200/50 dark:border-slate-800/40 relative z-10 bg-white/40 dark:bg-slate-900/40">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Access your ATS reports & apply for jobs, or{' '}
            <Link to="/applicant/register" className="font-semibold text-brand-600 hover:text-brand-500 dark:text-brand-400 dark:hover:text-brand-300 transition-colors">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-3 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-950 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 text-sm transition-all shadow-sm"
        ...
        <div className="mt-6 p-4 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100/50 dark:border-indigo-900/30 text-xs text-indigo-700 dark:text-indigo-400">
          <p className="font-bold mb-1">💡 Demo Applicant Account:</p>
          <ul className="list-disc list-inside space-y-0.5">
            <li><strong>Email:</strong> john.doe@gmail.com (Password: john123)</li>
          </ul>
        </div>
      </div>
    </div>
