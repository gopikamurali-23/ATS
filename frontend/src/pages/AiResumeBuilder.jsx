...
const AiResumeBuilder = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      {/* Toast Alert */}
      {message.text && (
        <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-2xl border shadow-xl transition-all duration-300 transform translate-y-0 ${message.type === 'success'
      ...
      <div className="bg-white dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/40 p-4 rounded-2xl flex items-center justify-between gap-2 overflow-x-auto shadow-sm">
        {[
          ...
          return (
            <div
              key={stage.key}
              className={`flex items-center gap-2 text-xs font-extrabold px-4 py-2.5 rounded-xl transition-all shrink-0 ${isCurrent
              ...
              {[1, 2, 3, 4, 5, 6].map((num) => {
                ...
                return (
                  <button
                    key={num}
                    onClick={() => setStep(num)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${step === num
              ...
              <div className="space-y-2">
                {RESUME_TEMPLATES.map((tmpl) => (
                  <div
                    ...
                    className={`p-3 border rounded-2xl cursor-pointer transition-all flex flex-col justify-between ${selectedTemplate === tmpl.id
        ...
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          <div className="glass p-8 rounded-3xl border border-slate-200/50 dark:border-slate-800/40 bg-white dark:bg-slate-900/60 shadow-md space-y-6">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all relative ${dragActive ? 'border-brand-500 bg-brand-50/10' : 'border-slate-300 dark:border-slate-800 hover:border-brand-400'
                }`}
            >
              <input
                type="file"
          ...
          <div className="glass bg-white dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/40 p-6 md:p-8 rounded-3xl space-y-8 shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
              ...
              <div className="md:col-span-4 flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-950/50 border border-slate-200/30 dark:border-slate-850 rounded-3xl text-center">
                ...
                <div className="relative flex items-center justify-center h-28 w-28 my-3">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle cx="56" cy="56" r="48"
                      className={`fill-transparent transition-all duration-1000 ${atsScoreDetails.overallScore >= 80 ? 'stroke-emerald-500' : atsScoreDetails.overallScore >= 60 ? 'stroke-amber-500' : 'stroke-red-500'
                        }`}
                      strokeWidth="8"
                ...
                <span className={`text-xs font-bold px-3 py-1 rounded-full ${atsScoreDetails.overallScore >= 80 ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400'
                  }`}>
