...
const AppContent = () => {
  ...
  // Navigation view state: 'home', 'jobs', 'portal'
  ...
  const handleOpenRoleSelect = () => {
    setAuthModalInitialMode('role-select');
    setIsAuthModalOpen(true);
  };
  ...
  const renderMainContent = () => {
        ...
        <LandingPage
          ...
          onSelectRole={(role) => {
            if (user) {
              ...
              setAuthModalInitialMode(role === 'recruiter' ? 'register-recruiter' : 'register-applicant');
              setIsAuthModalOpen(true);
            }
          }}
        />
    ...
    // 3. PROTECTED PORTAL VIEWS BASED ON AUTHENTICATED ROLE
    if (currentNav === 'portal' || user) {
      if (!user) {
        return (
      ...
      return (
        <div className="bg-white dark:bg-zinc-900 border border-rose-200 dark:border-rose-900/60 rounded-3xl p-12 text-center max-w-md mx-auto space-y-4 my-12 shadow-sm edge-glow-hover">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Return to Public Home
          </button>
        </div>
      );
    }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-zinc-950 font-sans text-slate-900 dark:text-zinc-100 flex flex-col antialiased transition-colors duration-200">
      ...
      <AuthModal
        ...
        onRoleSelected={() => setCurrentNav('portal')}
      />
