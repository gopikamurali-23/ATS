...
export const CandidatePortal = ({ onBackToHome }) => {
  ...
  // Domain Hooks (Architectural Rule 1 & 3: Decoupled Fetching & API handling)
  const { jobs, loading: jobsLoading } = useJobs();
  const { applications: myApplications, loading: appsLoading, applyToJob } = useApplications(false);

  const [savedJobIds, setSavedJobIds] = useState([2]);
  const [searchQuery, setSearchQuery] = useState('');
  const [locationFilter, setLocationFilter] = useState('');

  // Performance Optimization: Debounce keystroke and search filters (Architectural Rule 5)
  const debouncedSearchQuery = useDebounce(searchQuery, 300);
  const debouncedLocationFilter = useDebounce(locationFilter, 300);

  // Pagination for Jobs (Architectural Rule 5)
  ...
  const handleToggleSaveJob = (jobId) => {
    if (savedJobIds.includes(jobId)) {
      ...
      toastSuccess('Job removed from saved bookmarks');
    } else {
      setSavedJobIds([...savedJobIds, jobId]);
      toastSuccess('Job saved to your bookmarks');
    }
  };
  ...
  const handleApplySubmit = async (e) => {
    ...
    const result = applyJobSchema.safeParse({
      ...
      resumeText: applyResumeText.trim()
    });

    if (!result.success) {
      setApplyError(result.error.issues[0]?.message || 'Please provide a valid resume overview');
      return;
    }
    ...
    try {
      ...
      formData.append('resumeText', applyResumeText);

      await applyToJob(formData);
      toastSuccess(`Successfully applied for ${selectedJobForApply.title} at ${selectedJobForApply.companyName}!`);
      ...
      toastError("Failed to submit application: " + err.message);
    } finally {
      setIsApplying(false);
    }
  };
  ...
  const filteredJobs = jobs.filter(j => {
    ...
    const loc = debouncedLocationFilter.toLowerCase();
    const titleMatch = j.title.toLowerCase().includes(q) || (j.requiredSkills && j.requiredSkills.toLowerCase().includes(q));
    const locMatch = !loc || (j.location && j.location.toLowerCase().includes(loc));
    return titleMatch && locMatch;
  });

  const totalJobPages = Math.ceil(filteredJobs.length / jobsPerPage) || 1;
  const paginatedJobs = filteredJobs.slice((jobsPage - 1) * jobsPerPage, jobsPage * jobsPerPage);
    ...
    <div className="space-y-4">
      ...
      <div className="md:hidden bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-2 shadow-sm overflow-x-auto flex items-center gap-2 text-xs font-bold">
        {[
          ...
          { id: 'applications', label: `Applications (${myApplications.length})`, icon: CheckSquare },
          { id: 'analyzer', label: 'ATS Checker', icon: FileSearch },
          { id: 'builder', label: 'Resume Builder', icon: FileText },
          { id: 'interviews', label: `Interviews (${interviews.length})`, icon: Calendar },
          { id: 'saved-jobs', label: `Saved (${savedJobIds.length})`, icon: Bookmark },
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {jobsLoading ? (
                  ...
                  <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-8 text-center text-xs text-slate-500 dark:text-zinc-400">
                    No open job opportunities matched your current search filters. Try adjusting keywords or location.
                  </div>
                ) : (
                    ...
                    <div key={job.id} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 shadow-sm space-y-3 edge-glow-hover">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => { setSelectedJobForApply(job); setApplyError(''); }}
              ...
              {totalJobPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-200 dark:border-zinc-800 text-xs text-slate-500 dark:text-zinc-400">
                  <div>
                    Showing {(jobsPage - 1) * jobsPerPage + 1} to {Math.min(jobsPage * jobsPerPage, filteredJobs.length)} of {filteredJobs.length} open positions
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      disabled={jobsPage === 1}
                    ...
                    <span className="font-bold text-slate-800 dark:text-zinc-200">Page {jobsPage} of {totalJobPages}</span>
                    <button
                ...
                <form onSubmit={handleApplySubmit} className="space-y-4 text-xs">
                  ...
                  <div>
                    <label className="block font-semibold text-slate-700 dark:text-zinc-300 mb-1">
                      Attach Saved / Custom Resume Text for ATS Matching <span className="text-rose-500 font-bold ml-1">*</span>
                    </label>
                    <textarea
                      rows={6}
                      value={applyResumeText}
                      onChange={(e) => setApplyResumeText(e.target.value)}
                      placeholder="Paste or edit your resume text for ATS match scoring (min 10 characters)..."
                      className={`w-full px-3.5 py-2 border rounded-xl dark:bg-zinc-800 dark:text-white focus:outline-none focus:ring-2 font-mono ${
